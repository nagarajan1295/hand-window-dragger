...
def is_fist(landmarks, curled_threshold=3):
    """Rotation-invariant grab detector.

    A finger is "curled" when its tip sits closer to the wrist than its
    PIP joint does -- true regardless of how the hand is rotated toward
    the camera, unlike a simple tip.y < pip.y check.
    """
    wrist = landmarks[0]
    curled = 0
    for pip_idx, tip_idx in _FINGER_JOINTS:
        if _dist(landmarks[tip_idx], wrist) < _dist(landmarks[pip_idx], wrist):
            curled += 1
    return curled >= curled_threshold
