...
tutorial, sensitivity sliders, custom gesture management, and an activity
log.
"""
...
class App:
    def __init__(self, root):
        ...
        self._modal_open = False  # calibration or gesture-recording dialog owns the camera
    ...
    def _build_ui(self):
        ...
        self.preview_label.grid(row=0, column=0, rowspan=40, padx=(0, 12))
    ...
    def _on_calibration_finished(self, calibration):
        ...
        if calibration:
            ...
            self.mirror_var.set(calibration["mirror"])
            save_config(self.cfg)
            self._append_log(
        ...
        self._refresh_calibration_banner()
        # Sliders don't auto-refresh from cfg; rebuild the panel so the fist
        # sensitivity slider reflects the learned value.
    ...
    def _refresh_gesture_list(self):
        ...
        for t in load_templates():
            label = ACTIONS.get(t["action"], (t["action"], None))[0]
            self.gesture_list.insert("end", f"{t['name']}  ->  {label}")
